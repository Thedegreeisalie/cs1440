...
bool Triangle::isTriangle() const
{
    ...
    if (m_isValid)
    {
        std::cout << (!m_points[0]->isEquivalentTo(*(m_points[1]))) << std::endl;
        std::cout << (!m_points[0]->isEquivalentTo(*(m_points[2]))) << std::endl;
        std::cout << (!m_points[1]->isEquivalentTo(*(m_points[2]))) << std::endl;
        std::cout << (!m_points[0]->isEquivalentTo(*(m_points[1])) && !m_points[1]->isEquivalentTo(*(m_points[2])) && !m_points[2]->isEquivalentTo(*(m_points[0]))) << std::endl;
        ...
        {
            std::cout << (!m_edges[0]->isParallelTo(*(m_edges[1])) &&!m_edges[1]->isParallelTo(*(m_edges[2])) && !m_edges[2]->isParallelTo(*(m_edges[0]))) << std::endl;
...
char Triangle::getTriangleType() const
{
    ...
    if (isValid())
    {
        if (isTriangle()) //isTriangle returns true if it is a triangle false otherwise
        {
